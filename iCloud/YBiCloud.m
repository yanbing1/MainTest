...
+ (id)sharedCloud {
    static YBiCloud *sharedManager = nil;
...
- (void)saveAndCloseDocumentWithName:(NSString *)documentName withContent:(NSData *)content completion:(void (^)(UIDocument *cloudDocument, NSData *documentData, NSError *error))handler {
    ...
    YBiCloudDocument *document = [[YBiCloudDocument alloc] initWithFileURL:fileURL];
        ...
        for (int item = 0; item < [localDocuments count]; item++) {
            ...
            if (![[localDocuments objectAtIndex:item] hasPrefix:@"."]) {
                ...
                if (![self.previousQueryResults containsObject:[localDocuments objectAtIndex:item]]) {
                    ...
                    YBiCloudDocument *document = [[YBiCloudDocument alloc] initWithFileURL:cloudFileURL];
...
- (void)uploadLocalDocumentToCloudWithName:(NSString *)documentName completion:(void (^)(NSError *error))handler {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0ul), ^{
        ...
        if (![self.previousQueryResults containsObject:localDocument]) {
            ...
            // Create the UIDocument object from the URL
            YBiCloudDocument *document = [[YBiCloudDocument alloc] initWithFileURL:cloudURL];
            NSDate *cloudModDate = document.fileModificationDate;
...
- (YBiCloudDocument *)retrieveCloudDocumentObjectWithName:(NSString *)documentName {
    // Log Retrieval
...
+ (void)uploadLocalOfflineDocumentsWithDelegate:(id<YBiCloudDelegate>)delegate {
...
+ (void)updateFilesWithDelegate:(id<YBiCloudDelegate>)delegate {
